...
public class IPExt {
    ...
    public static void loadFromResource(String name) {
        try (InputStream inputStream = IPExt.class.getClassLoader().getResourceAsStream(name)) {
            final byte[] bytes = StreamUtils.toBytes(inputStream);
            loadFromBytes(bytes);
